...
static bool dualpi2_l_min_qlen(struct dualpi2_sched_data *q)
{
	return qdisc_qlen(q->l_queue) >= q->min_qlen_step;
}
			       ...
			       struct sk_buff **to_free)
{
	...
	if (skb_in_l_queue(skb)) {
		/* Apply step thresh if skb is L4S AND L-queue length >= min_qlen  */
		dualpi2_skb_cb(skb)->apply_step = skb_is_l4s(skb) && dualpi2_l_min_qlen(q);
				 ...
				 struct sk_buff **to_free)
{
	...
	if (q->split_gso && skb_is_gso(skb)) {
		...
		skb_list_walk_safe(nskb, nskb, next) {
			...
			/* Set pkt_len from segment length, copy classified
			 * and ect values from dualpi2_skb_cb, and set the
			 * ts value in dualpi2_enqueue_skb().
			 */
