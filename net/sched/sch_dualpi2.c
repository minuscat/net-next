...
static const struct netlink_range_validation dualpi2_nonzero_range = {
	.min = 1,
};

static const struct netlink_range_validation dualpi2_alpha_beta_range = {
	.min = 1,
	.max = ALPHA_BETA_MAX,
};

static const struct netlink_range_validation dualpi2_wc_range = {
	.min = 0,
	.max = MAX_WC,
};
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	if (tb[TCA_DUALPI2_STEP_THRESH]) {
		u32 step_thresh = nla_get_u32(tb[TCA_DUALPI2_STEP_THRESH]);
		WRITE_ONCE(q->step.thresh, step_thresh * NSEC_PER_USEC);
	}
